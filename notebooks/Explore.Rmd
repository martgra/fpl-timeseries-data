...
```{python}
elements_df_13.loc[elements_df_13['web_name'] == "Willian"][[ 'clean_sheets']].head(20)
```
...
```{python}
with open(Path("/home/vagrant/dev/fpl2021/data_description/elements_notes.json")) as json_file:
        data_dec = json.load(json_file)
```
...
```{python}
for i in dict(data_dec):
    print(i)
```

```{python}
x = {i:{"change": "None", "description": "None", "notes": "None", "data_type": "None", "Calculated": "None"} for i in sorted(_elements_columns)}
```
