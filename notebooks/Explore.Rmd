...
```{python}
elements_df_13.loc[elements_df_13['web_name'] == "Willian"][['web_name', 'assists']].head(20)
```
...
```{python}
sorted(_elements_columns)
```

```{python}
freekick = elements_df_13.loc[elements_df_13['corners_and_indirect_freekicks_order'].notnull()][[ 'web_name',
 'corners_and_indirect_freekicks_order',
 'corners_and_indirect_freekicks_text', 'team']]
freekick
freekick.loc[freekick["team"] == 7]
```

```{python}
elements_df_13.loc[elements_df_13['web_name'] == "Chilwell"][[  'clean_sheets',
 'code',
 'corners_and_indirect_freekicks_order',
 'corners_and_indirect_freekicks_text',
 'cost_change_event']].head(100)
```

```{python}
elements_df_14.loc[elements_df_14['web_name'] == "Willian"][[ 'clean_sheets']].head(20)
```

```{python}
with open(Path("/home/vagrant/dev/fpl2021/data_description/data_description.json")) as json_file:
        data_desc = json.load(json_file)

for i in data_desc["elements"]:
    data_desc["elements"][i]["type"="None"]
```

```{python}
print(json.dumps(data_desc, indent=4))
```
