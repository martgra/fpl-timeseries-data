...
class Wildcard:
    def __init__(self, dataframe, positions, population_size):
        ...
        self.players_sorted = [dataframe[0:self.positions[0]].sort_values("value",ascending=False).to_numpy(),
            dataframe[self.positions[0]:self.positions[1]].sort_values("value",ascending=False).to_numpy(),
            dataframe[self.positions[1]:self.positions[2]].sort_values("value",ascending=False).to_numpy(),
    ...
    def generate_teams(self):
        ...
        for j in range(self.population_size):
            ...
            for i in range(3):
                team.append(self.data[np.random.randint(self.positions[0],self.positions[1])].tolist())
            for i in range(4):
                team.append(self.data[np.random.randint(self.positions[1],self.positions[2])].tolist())
            for i in range(3):
                team.append(self.data[np.random.randint(self.positions[2],self.positions[3])].tolist())
            teams.append(team)
    ...
    def mutate(self,team,team_position):
        ...
        for i in range(substitutes):
            ...
            self.team_costs[team_position] = self.team_costs[team_position]+new_player[3] - player[3]
