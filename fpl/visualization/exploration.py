...
def reorder_elements(data_frame):
    try:
        ...
        _elements_columns.insert(0, _elements_columns.pop(
            _elements_columns.index("team")))
        _elements_columns.insert(0, _elements_columns.pop(
            _elements_columns.index("web_name")))
...
def compare_elements(snapshot_list, element_id, element_value):
    ...
    """
    _df_temp = snapshot_list[0].loc[snapshot_list[0]
                                    [element_id] == element_value]
    for i in snapshot_list:
        _df_temp = _df_temp.append(
            i.loc[i[element_id] == element_value], ignore_index=True)
    display(_df_temp)
    return(_df_temp)
